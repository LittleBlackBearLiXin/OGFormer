...
dataset = load_data(name='Citeseer',use_planetoid=True,use_geom=False,use_webkb=False,use_wikipedia_network=False)
data = dataset[0]
print(data)
...
class OGFormer(nn.Module):
    def __init__(self, input_dim):
        ...
        self.agg1 = MessgePsssing(input_dim, 64,SYSnoram=True)
        self.agg2 = MessgePsssing(64, 64,SYSnoram=True)
...
WEIGHT_DACAY = 1e-3
EPOCHS = 200
#Epoch 199 Average Test Accuracy: 0.7477,Average Test std: 0.0053,Attenton use sys noramlize
...
def train(iteration_seed):
    ...
    scheduler = ReduceLROnPlateau(optimizer, min_lr=1e-4, factor=0.6, patience=3, verbose=True)
    ...
    for epoch in range(EPOCHS):
        ...
        loss = cross_loss(logits[tensor_train_mask], tensor_y[tensor_train_mask])+klloss(h11,adj1,1e-5)+negiloss1*1e-4\
               +klloss(h22,adj2,1e-5)+negiloss2*1e-4
...
print("Train set size:", len(train_mask[train_mask == True]))
print("Validation set size:", len(val_mask[val_mask == True]))
print("Test set size:", len(test_mask[test_mask == True]))
print('============================================================================')

train_indices = set(torch.where(train_mask == True)[0].tolist())
val_indices = set(torch.where(val_mask == True)[0].tolist())
test_indices = set(torch.where(test_mask == True)[0].tolist())

train_val_intersection = train_indices & val_indices
train_test_intersection = train_indices & test_indices
val_test_intersection = val_indices & test_indices

print("Intersection between train and val sets:", len(train_val_intersection))
print("Intersection between train and test sets:", len(train_test_intersection))
print("Intersection between val and test sets:", len(val_test_intersection))

if len(train_val_intersection) == 0 and len(train_test_intersection) == 0 and len(val_test_intersection) == 0:
    print("No data leakage detected. No overlap between train, val, and test sets.")
else:
    print("Data leakage detected. There is overlap between some sets.")
